...
with tab_audit:
    # 1. ГРОМКИЙ ЮРИДИЧЕСКИЙ ДИСКЛЕЙМЕР
    st.markdown("""
        <div style="background-color: #ff4b4b22; border: 2px solid #ff4b4b; padding: 15px; border-radius: 10px; margin-bottom: 25px;">
            <h4 style="margin-top: 0; color: #ff4b4b; font-size: 1.2em;">⚖️ ВНИМАНИЕ: Юридический отказ от ответственности</h4>
            <p style="font-size: 0.9em; line-height: 1.4; margin-bottom: 0; color: #31333F;">
                Данный сервис работает на базе искусственного интеллекта и <b>не является юридической консультацией</b>. 
                ИИ может ошибаться. Результаты анализа носят ознакомительный характер. Перед принятием решений обязательно 
                <b>проконсультируйтесь с юристом</b>. Мы не несем ответственности за последствия использования этого инструмента.
            </p>
        </div>
    """, unsafe_allow_html=True)

    # Инициализация памяти, чтобы данные не пропадали после оплаты
    ...
    if file:
        ...
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # --- ТВОЯ ЛОГИКА ПОДБОРА ИНСТРУКЦИЙ (СОХРАНЕНА ПОЛНОСТЬЮ) ---
                ...
                # --- ТВОЙ ФИНАЛЬНЫЙ ПРОМПТ (СОХРАНЕН ПОЛНОСТЬЮ) ---
                prompt_instruction = (
                    ...
                    "Шаг 2: Для каждого риска проверь наличие пункта в договоре.\n"
                    "Шаг 3: Сформируй итоговый отчет.\n\n"
                ...
                # Сохраняем "сырой" ответ в сессию и принудительно обновляем страницу
                st.session_state.audit_result = response.choices[0].message.content
                st.rerun()

        # БЛОК Б: ОТОБРАЖЕНИЕ РЕЗУЛЬТАТОВ (ВНЕ КНОПКИ)
        # Этот блок работает ВСЕГДА, если в сессии есть данные (даже после оплаты и перезагрузки)
        if st.session_state.audit_result:
            raw_res = st.session_state.audit_result
            ...
            if "[PAYWALL]" in clean_res:
                ...
                # ПРОВЕРКА ОПЛАТЫ В URL (?paid=true)
                if st.query_params.get("paid") == "true":
                    st.success("✅ Оплата подтверждена! Вам открыт полный доступ к Протоколу разногласий.")
                    st.markdown(f"<div class='report-card'>{paid_part.strip()}</div>", unsafe_allow_html=True)
                    st.info("💡 Совет: Скопируйте таблицу выше и используйте её для переговоров.")
                else:
                    st.warning("🔒 Полный отчет и Протокол разногласий (готовые правки) доступны после оплаты.")
                    st.link_button("🚀 Оплатить Premium-доступ (850 ₽)", payment_url, use_container_width=True)
            else:
                # Если метки PAYWALL нет, выводим всё
