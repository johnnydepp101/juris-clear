    ...
    if file:
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                prompt_instruction = (
                    f"Действуй как опытный корпоративный юрист, специализирующийся на категории: {contract_type}. "
                    f"Твоя задача — максимально защитить интересы стороны: {user_role}. {special_instructions}\n"
                    "Составь профессиональный отчет со следующей структурой:\n"
                    "1. ОБЩИЙ ВЕРДИКТ (кратко: насколько документ безопасен для нашей стороны).\n"
                    "2. ФИНАНСОВЫЕ РИСКИ И ОТВЕТСТВЕННОСТЬ.\n"
                    "3. РИСКИ РАСТОРЖЕНИЯ И СПОРОВ.\n"
                    "Для каждого риска пиши: 'Суть условия' и 'Юридический анализ'. "
                    "Используй эмодзи 🔴, 🟠, 🟡. Будь предельно точен, избегай общих фраз.\n"
                    "В самом конце обязательно напиши фразу 'SCORE: X' (где X — число от 1 до 10).\n"
                    "Язык: Русский."
                )

                # 3. Вызов GPT-4o с расширенным контекстом
