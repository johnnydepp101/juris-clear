...
with tab_audit:
    ...
    if file:
        # --- БЛОК 1: ТОЛЬКО ЗАПУСК АНАЛИЗА ---
        # Мы оставляем здесь только логику получения данных
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # Твои инструкции (СОХРАНЕНО ПОЛНОСТЬЮ)
                special_instructions = ""
                if contract_type == "NDA": special_instructions = "Фокус на сроках конфиденциальности, исключениях и штрафах за разглашение."
                elif contract_type == "Аренда": special_instructions = "Фокус на индексации цены, условиях расторжения, возврате депозита и текущем ремонте."
                elif contract_type == "Трудовой": special_instructions = "Фокус на дисциплинарных взысканиях, условиях увольнения, обязанностях и мат. ответственности."
                elif contract_type == "ИТ-разработка": special_instructions = "Фокус на передаче исключительных прав на код, этапах приемки и гарантийном периоде."
                elif contract_type == "Поставка / Купля-продажа": special_instructions = "Фокус на переходе рисков, сроках поставки, штрафах за недопоставку и скрытых дефектах."
                elif contract_type == "Займ": special_instructions = "Фокус на процентах, очередности погашения, штрафах за просрочку и условиях досрочного возврата."
                elif contract_type == "Лицензионный": special_instructions = "Фокус на территории использования, объеме прав, сублицензировании и роялти."
                elif contract_type == "Агентский": special_instructions = "Фокус на порядке отчетности агента, расчете вознаграждении и праве на прямой поиск клиентов."

                prompt_instruction = (
                    "Будь строгим критиком. Если в договоре есть штрафы без вины или односторонние кабальные условия, оценка риска (SCORE) должна быть высокой (7-10). \n\n"
                    f"Действуй как опытный корпоративный юрист. Специализация: {contract_type}. Твоя задача — защитить интересы стороны: {user_role}. {special_instructions}\n\n"
                    "ИНСТРУКЦИЯ ДВОЙНОЙ ПРОВЕРКИ (Chain of Verification): Шаг 1: Проанализируй. Шаг 2: Проверь пункты. Шаг 3: Сформируй отчет.\n\n"
                    "СТРУКТУРА ОТВЕТА (ОБЯЗАТЕЛЬНО):\n## ⚖️ Юридический анализ рисков\n... [PAYWALL] ...\n## 🛠️ Протокол разногласий\nВ самом конце напиши 'SCORE: X' (1-10)."
                )

                response = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[{"role": "system", "content": prompt_instruction}, {"role": "user", "content": f"Текст договора:\n{text[:10000]}"}],
                    temperature=0.0
                )
                
                # ЗАПИСЫВАЕМ В ПАМЯТЬ И ПЕРЕЗАГРУЖАЕМ
                st.session_state.audit_result = response.choices[0].message.content
                st.rerun() 

        # --- БЛОК 2: ПОСТОЯННОЕ ОТОБРАЖЕНИЕ (ВНЕ КНОПКИ) ---
        # Этот код сработает и сразу после анализа, и после возврата с оплаты
        if st.session_state.audit_result:
            full_res = st.session_state.audit_result
            
            # Твой парсинг SCORE
            score_match = re.search(r"SCORE:\s*(\d+)", full_res)
            score = int(score_match.group(1)) if score_match else 5
            clean_res = re.sub(r"SCORE:\s*\d+", "", full_res).strip()
