    ...
    if file:
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # >>> START OF CHANGE (ПРОФЕССИОНАЛЬНЫЙ ПРОМПТ) <<<
                prompt_instruction = (
                    "Действуй как старший юридический партнер. Проанализируй текст договора и составь "
                    "профессиональный отчет. Структура отчета:\n"
                    "1. ОБЩИЙ ВЕРДИКТ (кратко о балансе интересов).\n"
                    "2. ФИНАНСОВЫЕ РИСКИ И ОТВЕТСТВЕННОСТЬ (подробно).\n"
                    "3. РИСКИ РАСТОРЖЕНИЯ И РАЗРЕШЕНИЯ СПОРОВ.\n"
                    "Для каждого риска укажи: 'Суть условия' и 'Юридический анализ'. "
                    "Используй эмодзи 🔴, 🟠, 🟡 для обозначения уровней угрозы.\n"
                    "В самом конце ответа обязательно напиши фразу 'SCORE: X' (где X — число от 1 до 10).\n"
                    "Язык: Русский."
                )
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Текст договора для анализа:\n{text[:5000]}"}
                    ],
                    temperature=0.2 # Делаем ответы более строгими и профессиональными
                )
                # >>> END OF CHANGE <<<
