    ...
    if file:
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # 2. ПОЛУЧАЕМ ВЕСЬ ТЕКСТ ОТ ИИ
                full_text = response.choices[0].message.content

                # 3. ВСТАВЛЯЕМ ЛОГИКУ РАЗДЕЛЕНИЯ (Вместо простого st.markdown)
                if "[PAYWALL]" in full_text:
                    # Разрезаем текст на две части
                    parts = full_text.split("[PAYWALL]")
                    free_part = parts[0]
                    paid_part = parts[1]

                    # ВЫВОДИМ БЕСПЛАТНУЮ ЧАСТЬ
                    st.markdown(free_part)
                    
                    st.divider() # Красивая линия-разделитель
                    
                    # ПРОВЕРЯЕМ ОПЛАТУ (Пока через кнопку-заглушку)
                    st.subheader("🔓 Платный раздел")
                    is_paid = st.checkbox("Открыть полный отчет (Имитация оплаты)")

                    if is_paid:
                        st.success("✅ Оплата подтверждена!")
                        st.markdown(paid_part) # Показываем скрытую таблицу
                        st.info("💡 Теперь вы можете скопировать правки для контрагента.")
                    else:
                        st.warning("⚠️ Таблица с готовыми правками доступна только в Premium-версии.")
                        st.button("Купить полный отчет за 850 ₽", type="primary")
                
                else:
                    # Если вдруг ИИ забыл написать [PAYWALL], просто выводим всё
