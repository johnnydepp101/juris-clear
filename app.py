...
# >>> START OF CHANGE 1 (Блок настроек) <<<
st.markdown("### ⚙️ Параметры анализа")
c1, c2 = st.columns(2)
with c1:
    user_role = st.selectbox("Ваша роль в договоре:", ["Заказчик (Покупатель)", "Исполнитель (Продавец)"])
with c2:
    contract_type = st.selectbox("Тип документа:", ["Авто-определение", "Договор услуг", "NDA / Конфиденциальность", "Аренда", "Поставка / Купля-продажа"])
# >>> END OF CHANGE 1 <<<
    ...
    if file:
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # >>> START OF CHANGE 2 (GPT-4o + Динамический промпт) <<<
                # Формируем уточненную инструкцию
                prompt_instruction = (
                    f"Действуй как опытный корпоративный юрист. Проанализируй этот документ: {contract_type}. "
                    f"Твоя задача — защитить интересы стороны: {user_role}. "
                    "Составь профессиональный отчет со следующей структурой:\n"
                    "1. ОБЩИЙ ВЕРДИКТ (насколько договор безопасен для нашей стороны).\n"
                    "2. ФИНАНСОВЫЕ РИСКИ И ОТВЕТСТВЕННОСТЬ.\n"
                    "3. РИСКИ РАСТОРЖЕНИЯ И СПОРОВ.\n"
                    "Для каждого риска пиши: 'Суть условия' и 'Юридический анализ'. "
                    "Используй 🔴, 🟠, 🟡. Будь предельно точен, избегай общих фраз. "
                    "В самом конце обязательно напиши 'SCORE: X' (X от 1 до 10).\n"
                    "Язык: Русский."
                )

                # Вызываем самую мощную модель GPT-4o
                response = client.chat.completions.create(
                    model="gpt-4o",  # МЫ ОБНОВИЛИ МОДЕЛЬ ЗДЕСЬ
                    messages=[
                        {"role": "system", "content": prompt_instruction},
                        {"role": "user", "content": f"Текст договора:\n{text[:8000]}"} # Увеличили лимит до 8к символов
                    ],
                    temperature=0.1 # Еще ниже температура для максимальной точности
                )
                # >>> END OF CHANGE 2 <<<
