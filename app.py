    ...
    if file:
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                # --- ЛОГИКА ОТОБРАЖЕНИЯ ПОСЛЕ АНАЛИЗА ---
if "[PAYWALL]" in clean_res:
    ...
    # 2. ПРОВЕРЯЕМ ПАРАМЕТР ОПЛАТЫ В URL
    if st.query_params.get("paid") == "true":
        # Если клиент вернулся с ?paid=true
        st.success("✅ Оплата подтверждена! Вам открыт полный доступ к правкам.")
        st.markdown(f"<div class='report-card'>{paid_part.strip()}</div>", unsafe_allow_html=True)
        # Добавляем кнопку скачивания (подготовим её следующим шагом)
        st.button("📥 Скачать Протокол в Word") 
    else:
        # Если оплаты еще нет — показываем ОДНУ кнопку
        st.divider()
        st.warning("🔒 Таблица с готовыми правками и Протокол разногласий доступны в Premium-версии.")
